...
for(model in name_models){
  ...
  for(itimescale in 1:ntimescale){
    ...
    write.table(mat_10yrRL, paste0("D:/EUROCORDEX_extremes/10yrRL_BenPosch/matrix_10yrRL_", ts, "hr_", model, ".csv"), 
    sep = ",", col.names = FALSE, row.names = FALSE)
    write.table(mat_shape, paste0("D:/EUROCORDEX_extremes/10yrRL_BenPosch/matrix_shape_", ts, "hr_", model, ".csv"), 
    sep = ",", col.names = FALSE, row.names = FALSE)
  }
}

#####################################################################################################################
# STEP 2: seasonal 10-year return level, different temporal  and spatial scales
#####################################################################################################################
